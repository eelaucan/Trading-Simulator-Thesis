...
export class TradePlannerApp {
    ...
    renderForm(props, remainingSlots) {
        ...
        const actionOptions = [
            ...
            .map((value) => `<option value="${value}" ${actionType === value ? "selected" : ""}>${escapeHtml(ACTION_LABELS[value])}</option>`)
            .join("");
        const tickerField = actionType === "hold"
          ...
          <label class="tp-field">
            ...
            <select id="tp-ticker" ${tickerOptions.length === 0 ? "disabled" : ""}>
              ${tickerOptions
                .map((ticker) => `<option value="${escapeHtml(ticker)}" ${this.formState.ticker === ticker ? "selected" : ""}>${escapeHtml(ticker)}</option>`)
                .join("")}
            </select>
    ...
    renderSizingFields(actionType) {
        ...
        <label class="tp-field">
          ...
          <select id="tp-quantity-type">
            ${["shares", "notional_dollars", "nav_fraction"]
                .map((value) => `<option value="${value}" ${this.formState.quantityType === value ? "selected" : ""}>${escapeHtml(QUANTITY_LABELS[value])}</option>`)
                .join("")}
          </select>
        </label>
        <label class="tp-field">
      ...
      `;
        ...
        <label class="tp-field">
          ...
          <select id="tp-quantity-type">
            ${["shares", "notional_dollars", "close_all"]
                .map((value) => `<option value="${value}" ${this.formState.quantityType === value ? "selected" : ""}>${escapeHtml(QUANTITY_LABELS[value])}</option>`)
                .join("")}
          </select>
        </label>
    ...
    renderPlanReview(props) {
        if (this.currentPlan.length === 0) {
            const pendingCopy = props.pending_liquidations.length > 0
    ...
    renderImpactPreview(preview, previewIsSynced) {
      ...
      `;
        ...
        const largestWeight = typeof preview.projected_max_weight === "number"
    ...
    attachListeners() {
        ...
        actionTypeInput?.addEventListener("change", (event) => {
            this.formState.actionType = event.currentTarget
                .value;
        ...
        quantityTypeInput?.addEventListener("change", (event) => {
            this.formState.quantityType = event.currentTarget
                .value;
            this.formState.error = null;
            this.render();
        });
...
function summarizeAction(action) {
    if (action.action_type === "buy") {
        ...
        if (action.quantity_type === "notional_dollars" &&
