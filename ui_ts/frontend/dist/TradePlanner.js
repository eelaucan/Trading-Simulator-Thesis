...
export class TradePlannerApp {
  ...
  render() {
    ...
    const remainingSlots = Math.max(0, props.max_actions_per_step - this.currentPlan.length);
    const previewIsSynced = this.planSignature(this.currentPlan) === this.planSignature(props.current_batch);
    this.root.innerHTML = `
          ...
          <div class="tp-panel tp-panel--review">
            ...
            <div class="tp-submit-area">
              ...
              <div class="tp-submit-actions">
                <button class="tp-button tp-button--ghost" id="tp-clear-plan" ${this.currentPlan.length === 0 || props.disabled ? "disabled" : ""}>
                  Clear plan
                </button>
                <button class="tp-button tp-button--primary" id="tp-submit-plan" ${props.disabled ? "disabled" : ""}>
  ...
  renderForm(props, remainingSlots) {
    const actionType = this.formState.actionType;
    const tickerOptions = this.getTickerOptions(actionType, props);
    const disableAdd = remainingSlots <= 0 || tickerOptions.length === 0 || Boolean(props.disabled);
    const actionOptions = ["buy", "sell", "reduce", "set_stop", "remove_stop"].map((value) => `<option value="${value}" ${actionType === value ? "selected" : ""}>${escapeHtml(ACTION_LABELS[value])}</option>`).join("");
    const tickerField = actionType === "hold" ? "" : `
          <label class="tp-field">
            <span class="tp-label">Choose the stock</span>
            <select id="tp-ticker" ${tickerOptions.length === 0 ? "disabled" : ""}>
              ${tickerOptions.map((ticker) => `<option value="${escapeHtml(ticker)}" ${this.formState.ticker === ticker ? "selected" : ""}>${escapeHtml(ticker)}</option>`).join("")}
            </select>
            ${tickerOptions.length === 0 ? `<span class="tp-field-note">${escapeHtml(this.emptyTickerMessage(actionType))}</span>` : this.renderTickerContext(this.formState.ticker, props)}
          </label>
        `;
    ...
    return `
      ...
      ${tickerField}
      ${sizingField}
      ${this.formState.error ? `<div class="tp-inline-error">${escapeHtml(this.formState.error)}</div>` : ""}
      <button class="tp-button tp-button--secondary" id="tp-add-action" ${disableAdd ? "disabled" : ""}>
        Add this decision to your weekly plan
      </button>
    `;
  }
  renderSizingFields(actionType) {
    if (actionType === "buy") {
      return `
        <label class="tp-field">
          <span class="tp-label">How would you like to size this purchase?</span>
          <select id="tp-quantity-type">
            ${["shares", "notional_dollars", "nav_fraction"].map((value) => `<option value="${value}" ${this.formState.quantityType === value ? "selected" : ""}>${escapeHtml(QUANTITY_LABELS[value])}</option>`).join("")}
          </select>
        </label>
        <label class="tp-field">
          <span class="tp-label">${escapeHtml(this.quantityInputLabel(this.formState.quantityType))}</span>
          <input id="tp-quantity" type="number" min="0.0001" step="0.01" value="${escapeHtml(this.formState.quantityText)}" />
        </label>
      `;
    }
    if (actionType === "sell") {
      const quantitySelector = `
        <label class="tp-field">
          ...
          <select id="tp-quantity-type">
            ${["shares", "notional_dollars", "close_all"].map((value) => `<option value="${value}" ${this.formState.quantityType === value ? "selected" : ""}>${escapeHtml(QUANTITY_LABELS[value])}</option>`).join("")}
          </select>
        </label>
      `;
    ...
    if (actionType === "reduce") {
      return `
        <label class="tp-field">
          ...
          <input id="tp-fraction" type="number" min="0.0001" max="1" step="0.01" value="${escapeHtml(this.formState.fractionText)}" />
    ...
    if (actionType === "set_stop") {
      return `
        <label class="tp-field">
          ...
          <input id="tp-stop-price" type="number" min="0.01" step="0.01" value="${escapeHtml(this.formState.stopPriceText)}" />
  ...
  renderTickerContext(ticker, props) {
    if (!ticker) {
      return "";
    }
    ...
    return parts.length > 0 ? `<span class="tp-field-note">${escapeHtml(parts.join(" "))}</span>` : "";
  }
  renderPlanReview(props) {
    if (this.currentPlan.length === 0) {
      const pendingCopy = props.pending_liquidations.length > 0 ? "Earlier forced sales would still execute if they were already scheduled." : "If you submit now, your current portfolio simply carries into next week.";
      return `
    ...
    return `
      <div class="tp-plan-list">
        ${this.currentPlan.map((action, index) => {
          const detail = action.detail ? `<div class="tp-plan-detail">${escapeHtml(action.detail)}</div>` : "";
          return `
  ...
  renderImpactPreview(preview, previewIsSynced) {
    if (this.currentPlan.length === 0) {
      return "";
    }
    if (!previewIsSynced) {
    ...
    const largestWeight = typeof preview.projected_max_weight === "number" ? formatPercent(preview.projected_max_weight) : "N/A";
    const warnings = preview.warnings.length ? `
      ...
      `;
    const notes = preview.notes.length ? `<div class="tp-preview-notes">${preview.notes.map((note) => `<div>${escapeHtml(note)}</div>`).join("")}</div>` : "";
    return `
  ...
  attachListeners() {
    ...
    const actionTypeInput = this.root.querySelector("#tp-action-type");
    actionTypeInput?.addEventListener("change", (event) => {
      this.formState.actionType = event.currentTarget.value;
      this.formState.error = null;
      this.syncFormState();
      this.render();
    });
    const tickerInput = this.root.querySelector("#tp-ticker");
    tickerInput?.addEventListener("change", (event) => {
      this.formState.ticker = event.currentTarget.value;
      this.render();
    });
    const quantityTypeInput = this.root.querySelector("#tp-quantity-type");
    quantityTypeInput?.addEventListener("change", (event) => {
      this.formState.quantityType = event.currentTarget.value;
      this.formState.error = null;
      this.render();
    });
    const quantityInput = this.root.querySelector("#tp-quantity");
    quantityInput?.addEventListener("input", (event) => {
      this.formState.quantityText = event.currentTarget.value;
    });
    const fractionInput = this.root.querySelector("#tp-fraction");
    fractionInput?.addEventListener("input", (event) => {
      this.formState.fractionText = event.currentTarget.value;
    });
    const stopPriceInput = this.root.querySelector("#tp-stop-price");
    stopPriceInput?.addEventListener("input", (event) => {
      this.formState.stopPriceText = event.currentTarget.value;
    });
    const addButton = this.root.querySelector("#tp-add-action");
    ...
    const clearButton = this.root.querySelector("#tp-clear-plan");
    ...
    const submitButton = this.root.querySelector("#tp-submit-plan");
    submitButton?.addEventListener("click", () => {
      this.emit("submit");
    });
    this.root.querySelectorAll("[data-remove-index]").forEach((button) => {
  ...
  syncFormState() {
    ...
    const availableActions = ["buy", "sell", "reduce", "set_stop", "remove_stop"];
  ...
  getTickerOptions(actionType, props) {
    return props.ticker_options[actionType] ?? [];
  }
  emptyTickerMessage(actionType) {
    if (actionType === "reduce") {
  ...
  quantityInputLabel(quantityType) {
...
function summarizeAction(action) {
  if (action.action_type === "buy") {
    ...
    if (action.quantity_type === "notional_dollars" && typeof action.quantity === "number") {
